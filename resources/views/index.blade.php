...
@section('head')
    ...
    <style>
        ...
        .hero-float-2 {
            top: -16px;
            right: -16px;
            animation: floatCard 3s ease-in-out infinite .5s
        }
        ...
        .deal-overlay {
            position: absolute;
            inset: 0;
            background: linear-gradient(135deg, rgba(10, 15, 30, .85) 0%, rgba(10, 15, 30, .4) 100%);
            padding: 32px
        }
...
@section('content')
    ...
    <section class="hero">
        ...
        <div class="hero-inner">
            <div>
                <div class="hero-tag"><i class="fi fi-rr-sparkles"></i> Toko Elektronik Terpercaya</div>
                ...
                <div style="display: flex; gap: 12px; flex-wrap: wrap">
                    <a href="{{ route('products.index') }}" class="btn btn-primary btn-lg"
                        style="display:inline-flex;align-items:center;gap:8px"><i class="fi fi-rr-shopping-bag"></i> Belanja
                        Sekarang</a>
                ...
                <div class="hero-stats">
                    ...
                    <div class="hero-stat">
                        <div class="hero-stat-num">5.0<i class="fi fi-rr-star"
                                style="font-size:20px;color:#fbbf24;margin-left:4px"></i></div>
            ...
            <div class="hero-img-col">
                ...
                <div class="hero-float-card hero-float-1">
                    <div class="hfc-icon" style="background: #eef3ff"><i class="fi fi-rr-truck-side"
                ...
                <div class="hero-float-card hero-float-2">
                    <div class="hfc-icon" style="background: #dcfce7"><i class="fi fi-rr-shield-check"
    ...
    <section class="cat-section">
        <div class="container">
            <div class="section-header">
                <div>
                    <div class="section-tag"><i class="fi fi-rr-apps"></i> Kategori</div>
            ...
            <div class="cat-scroll">
                @foreach ($kategoris as $kategori)
                    <a href="{{ route('products.index', ['kategori' => $kategori->nama_kategori]) }}" class="cat-card">
                        <div class="cat-icon"><i class="fi fi-rr-box"></i></div>
    ...
    <section class="featured-section">
        <div class="container">
            <div class="section-header">
                <div>
                    <div class="section-tag"><i class="fi fi-rr-star"></i> Unggulan</div>
                        ...
                        <div class="prod-card" style="text-decoration:none;color:inherit;display:block">
                            @auth
                                @if (auth()->user()->role === 'customer')
                                    <button class="wishlist-btn {{ in_array($item->produk->id_produk, $wishlistIds) ? 'liked' : '' }}"
                                        data-id="{{ $item->produk->id_produk }}" title="Tambah ke Wishlist"
                                        onclick="toggleWishlist(this, {{ $item->produk->id_produk }}, event)">
                                        @if(in_array($item->produk->id_produk, $wishlistIds))
                                            <i class="fi fi-rr-heart" style="color:var(--danger)"></i>
                                        @else
                                ...
                                @endif
                            @endauth
                            <a href="{{ route('products.show', $item->produk) }}" style="text-decoration:none;color:inherit">
    ...
    @if ($produkTerlaris->count() >= 2)
        <section class="deals-section">
            <div class="container">
                <div class="section-header">
                    <div>
                        <div class="section-tag"><i class="fi fi-rr-badge-percent"></i> Penawaran</div>
                ...
                <div class="deals-grid">
                    @foreach ($produkTerlaris->take(2) as $item)
                        <div class="deal-card">
                            <img src="{{ asset('storage/products/' . $item->produk->gambar) }}"
                                alt="{{ $item->produk->nama_produk }}" loading="lazy" decoding="async" />
                            <div class="deal-overlay" @if (!$loop->first)
                            style="background: linear-gradient(135deg, rgba(14, 165, 160, 0.85) 0%, transparent 100%);" @endif>
                                <div class="deal-tag"><i class="fi fi-rr-flame"></i> Penawaran Terlaris</div>
                                <div class="deal-title">{{ $item->produk->nama_produk }}</div>
                                <div class="deal-sub">
                                    {{ $item->produk->kategori->nama_kategori ?? 'Kategori' }} — Terjual {{ $item->total_terjual }}
                                    item
                                </div>
    ...
    <section class="new-section">
        <div class="container">
            <div class="section-header">
                <div>
                    <div class="section-tag"><i class="fi fi-rr-sparkles"></i> Terbaru</div>
            ...
            <div class="prod-grid">
                @foreach ($produkBaru as $produk)
                    <div class="prod-card" style="text-decoration:none;color:inherit;display:block">
                        @auth
                            @if (auth()->user()->role === 'customer')
                                <button class="wishlist-btn {{ in_array($produk->id_produk, $wishlistIds) ? 'liked' : '' }}"
                                    data-id="{{ $produk->id_produk }}" title="Tambah ke Wishlist"
        ...
        <div class="container">
            <div style="text-align: center">
                <div class="section-tag" style="background: rgba(255, 255, 255, 0.1); color: rgba(255, 255, 255, 0.8);">
                    <i class="fi fi-rr-bolt" style="color:var(--blue)"></i> Kenapa Elektronik Modern
                </div>
            ...
            <div class="why-grid">
                <div class="why-card">
                    <div class="why-icon"><i class="fi fi-rr-shield-check"></i></div>
                    <div class="why-title">Produk 100% Original</div>
                    <div class="why-text">Semua produk bersumber langsung dari distributor resmi dengan garansi pabrik.
                    </div>
                </div>
                <div class="why-card">
                    <div class="why-icon"><i class="fi fi-rr-truck-side"></i></div>
                ...
                <div class="why-card">
                    <div class="why-icon"><i class="fi fi-rr-credit-card"></i></div>
                ...
                .then(data => {
                    if (data && typeof data.liked !== 'undefined') {
                        if (data.liked) {
                            ...
                            btn.innerHTML = '<i class="fi fi-rr-heart" style="color:var(--danger)"></i>';
                        } else {
